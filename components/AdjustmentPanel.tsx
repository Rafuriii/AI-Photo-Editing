...
const AdjustmentPanel: React.FC<AdjustmentPanelProps> = ({ onApplyAdjustment, isLoading, selectedImageCount }) => {
  ...
  const presets = [
    ...
    { name: 'Cahaya Hangat', prompt: 'Adjust the color temperature to give the image warmer, golden-hour style lighting.' },
  ...
  const buttonText = selectedImageCount > 1
    ? `Terapkan pada ${selectedImageCount} Gambar`
      ...
      {activePrompt && (
        <div className="animate-fade-in flex flex-col gap-4 pt-2">
            <button
                ...
                disabled={isLoading || !activePrompt.trim() || selectedImageCount === 0}
            >
                {buttonText}
            </button>
        </div>
